...
const NotificationDialog = ({
  ...
  // Prevent hydration mismatch from Radix Dialog's auto-generated IDs
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  return (
    <>
      {!mounted ? (
        ...
        <Button variant={"outline"} className="bg-zinc-950 cursor-pointer rounded-lg" disabled>
        ...
        <Dialog>
          <DialogTrigger asChild className="cursor-pointer bg-white rounded-lg">
            {
              <Button asChild variant={"outline"} className="bg-zinc-950">
                {
                  hasUnread ? (
